...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    printAndBuffer("Camera Ready! Use http://", false);
    printAndBuffer(WiFi.localIP().toString());
  }
  printAndBuffer("SoftAP IP: http://", false);
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    filename += ".jpg";
    takePhoto(filename);
    printAndBuffer(String(String(gps.location.lat(), 6) + ", " + String(gps.location.lng(), 6)));
  }
